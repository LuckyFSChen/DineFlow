...
@php
function kitchenStoreRouteValue($value) {
...
$storeRoute = kitchenStoreRouteValue($store);
                ...
                <div class="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-800 px-2 py-1.5 text-xs">
                    ...
                    <select
                        ...
                        @foreach($availableStores as $availableStore)
                            <option value="{{ route('admin.stores.kitchen', ['store' => kitchenStoreRouteValue($availableStore)]) }}" @selected((int) $availableStore->id === (int) $store->id)>
                                {{ $availableStore->name }}
                            </option>
                        @endforeach
                    </select>
