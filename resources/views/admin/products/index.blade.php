                    ...
                    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500">
                                <span data-category-item-count>{{ $category->products->count() }}</span> {{ __('admin.products_items_count_suffix') }}
                                ・ {{ __('admin.products_sort_label') }} {{ $category->sort ?? 1 }}
                                @if(!empty($category->prep_time_minutes))
                                    ・ {{ __('admin.products_form_category_prep_time_minutes') }} {{ (int) $category->prep_time_minutes }} {{ __('admin.products_prep_time_minutes_unit') }}
                                @endif
                            </p>
                        </div>
                    ...
                    <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                        @foreach($inactiveCategories as $inactiveCategory)
                            <article class="rounded-2xl border border-amber-200 bg-white p-4">
                                ...
                                <p class="mt-1 text-sm text-slate-500">
                                    {{ __('admin.products_sort_label') }} {{ $inactiveCategory->sort ?? 1 }}
                                    ・ {{ $inactiveCategory->products_count }} {{ __('admin.products_items_count_suffix') }}
                                    @if(!empty($inactiveCategory->prep_time_minutes))
                                        ・ {{ __('admin.products_form_category_prep_time_minutes') }} {{ (int) $inactiveCategory->prep_time_minutes }} {{ __('admin.products_prep_time_minutes_unit') }}
                                    @endif
                                </p>
    ...
    <div class="w-full max-w-xl rounded-3xl bg-white shadow-2xl">
        ...
        <form id="category-modal-form" class="px-6 py-5">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-600">{{ __('admin.products_form_category_prep_time_minutes') }}</label>
                    <input type="number" name="prep_time_minutes" min="1" max="300" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="15">
                    <p class="mt-1 text-xs text-slate-500">{{ __('admin.products_form_category_prep_time_minutes_hint') }}</p>
                </div>
            </div>
...
(() => {
    ...
    const setCategoryFormValues = (category = null) => {
        ...
        if (!category) {
            ...
            categoryModalForm.elements['prep_time_minutes'].value = '';
            return;
        }

        categoryModalForm.elements['name'].value = category.name ?? '';
        categoryModalForm.elements['sort'].value = category.sort ?? 1;
        categoryModalForm.elements['prep_time_minutes'].value = category.prep_time_minutes ?? '';
    };
